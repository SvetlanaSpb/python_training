...
class ContactHelper:
    ...
    def get_contact_list(self):
        ...
        # строим объект с информацией о контакте contacts = []
            ...
            # получаем данные из ячеек
                ...
                # так как в ячейке телефонов отдельные телефоны не указаны приходится получать информацию по всей ячейке
                # а потом порезать её на части
                all_phones = cells[5].text
                # теперь это список телефонов у ячейки берём текст а потом делим его на телефоны
            ...
            # параметры для конструирования нового объекта contacts
                self.contact_cache.append(New(firstname=firstname, lastname=lastname, id=id,
    ...
    def get_contact_info_from_edit_page(self, index):
        ...
        return New(firstname=firstname, lastname=lastname,id=id, homephone=homephone,
